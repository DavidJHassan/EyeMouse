...
while(1):
	...
	if flag:
		...
		nypt = []
		mask = np.array([])
		for (x,y,w,h) in faces:#find face
			...
			for (ex,ey,ew,eh) in eyes:#Find one eye. We want to do calculations using on eye for simplicity
				mask[ey:ey+eh, ex:ex+ew] = roi_gray[ey:ey+eh, ex:ex+ew]
				center.append(mask[ey+eh/2, ex+ew/2])
				for i in range(1,10):#Grab some points around the eyes
			...
			#average the points around the eyes to try and determine the eyes looking direction
			avgx = np.mean(xpt) 
			avgnx = np.mean(nxpt) 
			avgy = np.mean(ypt) - 5
			avgny = np.mean(nypt)
			...
			#Try and make sure that the averages are different enough to be signicant based on some arbitary value 10
			if (avgx - avgnx) < -10:
				mx -= 5
			elif (avgx - avgnx) > 10:
				mx += 5
